...
class SearchQueryTest < Minitest::Test
  def test_defaults
    ...
    assert_empty(query.media_types, "Wrong default media types")
  ...
  def test_add_offset
    offset_to_add = 100
    query = SearchQuery.new
    query.add_offset offset_to_add
  ...
  def test_add_platform
    ...
    query = SearchQuery.new
    query.add_platform platform_to_add
  ...
  def test_add_media_type
    media_to_add = :retail
    query = SearchQuery.new
    query.add_media_type media_to_add
    assert_includes(query.media_types, media_to_add, "#{media_to_add.to_s.capitalize} not added in media platforms")
  end

  def test_add_genre
    genre_to_add = :action
    query = SearchQuery.new
    query.add_genre genre_to_add
  ...
  def test_add_dlc_available
    query = SearchQuery.new
    query.add_dlc_available
